...
float Delay_apply(Delay* del, float sample) {
	...
	if (del->changingDelay) {
		...
		// increment new read pointer, bounds check
		del->newReadPtr++;
		if (del->newReadPtr >= del->buffer + del->buffSize) {
			del->newReadPtr -= del->buffSize;
		}
	...
	// write to delay line
	*del->writePtr = sample;
	// increment rea/write pointers, bounds check
	del->readPtr++;
	if (del->readPtr >= del->buffer + del->buffSize) {
		del->readPtr -= del->buffSize;
	}
	del->writePtr++;
